    ...
    @if($edittodoID ==$todo->id)
         <input wire:model="editTodoName" type="text" placeholder="Todo.."
                ...
                @error('editTodoName')
                    ...
                    @enderror()
                    <button wire:click="update" class="px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600 mt-2">Update</button>
                    <button wire:click="$set('edittodoID', null)" class="px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600 mt-2">Cancel</button>
                    @else
